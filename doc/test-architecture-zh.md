...
- Duration/tick 转换
- EventGroup 位等待
- Queue close 生命周期（`close()` 后操作返回 `Error::QueueClosed`）
...
- 精确的 tick 计数
- ISR 安全 API
- Thread 协作式取消（`delete` 设置标志；不强制终止）
- Thread join 语义（`join()` 是 Linux 扩展，不在 FreeRTOS trait 中）
- Mutex<T> 非递归行为（同线程二次加锁返回 `Error::MutexLockFailed`）