...
use crate::traits::{BytesHasLen, QueueFn, QueueStreamedFn, SystemFn, ToTick};
...
impl QueueFn for Queue {
    ...
    fn fetch(&self, buffer: &mut [u8], time: TickType) -> Result<()> {
        let ret = unsafe { xQueueReceive(self.0, buffer.as_mut_ptr() as *mut c_void, time) };
        if ret == 0 {
            Err(Error::Timeout)
        } else {
            Ok(())
    ...
    fn post(&self, item: &[u8], time: TickType) -> Result<()> {
        let ret = xQueueSendToBack!(self.0, item.as_ptr() as *const c_void, time);
...
impl Debug for Queue {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("Queue").field("handle", &self.0).finish()
    }
}
