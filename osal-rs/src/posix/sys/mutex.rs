...
use libc::{pthread_mutex_init, pthread_mutex_destroy, pthread_mutex_lock, pthread_mutex_trylock, pthread_mutex_unlock};
use libc::{pthread_mutex_t, pthread_mutexattr_t, pthread_mutexattr_init, pthread_mutexattr_settype, pthread_mutexattr_destroy};
use libc::{PTHREAD_MUTEX_RECURSIVE, PTHREAD_MUTEX_ERRORCHECK};
...
impl PosixMutex {
    pub fn new(kind: i32) -> Option<Self> {
        ...
        if unsafe { pthread_mutexattr_init(&mut attr) } != 0 { return None; }
        ...
        unsafe { pthread_mutexattr_destroy(&mut attr) };
        if ret == 0 { Some(Self { raw: UnsafeCell::new(mtx) }) } else { None }
    }
