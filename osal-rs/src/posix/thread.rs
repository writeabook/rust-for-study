...
///
/// The first thread that reaches this initializer becomes the synthetic
/// "main" thread for the POSIX backend.  The id is allocated inside the
/// `pthread_once` initializer so concurrent callers cannot race through a
/// shared pending-id slot.
///
...
pub(crate) fn ensure_main_thread_registered() {
    ...
    let core = main_thread_core(); // Returns Arc<ThreadCore>; init is idempotent.

    // Bind this pthread to the synthetic main core.
    set_current_thread_id(core.id);
    register_thread(core.id, &core);
}
