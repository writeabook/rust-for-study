...
impl Queue {
    pub fn new(size: UBaseType, message_size: UBaseType) -> Result<Self> {
        ...
        Ok(Self {
            handle: mtx.raw_ptr() as QueueHandle, mtx, not_empty, not_full,
            buffer: UnsafeCell::new(VecDeque::with_capacity(size as usize)),
            capacity: size as usize, message_size: message_size as usize,
    ...
    #[inline] pub fn fetch_with_to_tick(&self, b: &mut [u8], t: impl ToTick) -> Result<()> { self.fetch(b, t.to_ticks()) }
    #[inline] pub fn post_with_to_tick(&self, i: &[u8], t: impl ToTick) -> Result<()> { self.post(i, t.to_ticks()) }
}
...
impl QueueFn for Queue {
    fn post(&self, item: &[u8], time: TickType) -> Result<()> {
        ...
        if *closed { let _ = self.mtx.unlock(); return Err(Error::QueueClosed); }
        if item.len() != self.message_size { let _ = self.mtx.unlock(); return Err(Error::InvalidMessageSize); }
        ...
        if time == 0 { let _ = self.mtx.unlock(); return Err(Error::Timeout); }
        ...
        loop {
            ...
            if !self.not_full.timedwait(&self.mtx, &deadline) { let _ = self.mtx.unlock(); return Err(Error::Timeout); }
        }
    }

    fn post_from_isr(&self, item: &[u8]) -> Result<()> {
        if !self.mtx.try_lock() { return Err(Error::Timeout); }
        let closed = unsafe { &*self.closed.get() };
        if *closed { let _ = self.mtx.unlock(); return Err(Error::QueueClosed); }
        if item.len() != self.message_size { let _ = self.mtx.unlock(); return Err(Error::InvalidMessageSize); }
        let buffer = unsafe { &mut *self.buffer.get() };
        if buffer.len() < self.capacity {
            ...
            let _ = self.mtx.unlock();
            Ok(())
        } else { let _ = self.mtx.unlock(); Err(Error::Timeout) }
    }
    ...
    fn fetch(&self, out: &mut [u8], time: TickType) -> Result<()> {
        ...
        let closed = unsafe { &*self.closed.get() };
        if *closed { let _ = self.mtx.unlock(); return Err(Error::QueueClosed); }
        if out.len() != self.message_size { let _ = self.mtx.unlock(); return Err(Error::InvalidMessageSize); }

        let buffer = unsafe { &mut *self.buffer.get() };
        if let Some(item) = buffer.pop_front() { out.copy_from_slice(&item); self.not_full.signal(); let _ = self.mtx.unlock(); return Ok(()); }
        if time == 0 { let _ = self.mtx.unlock(); return Err(Error::Timeout); }
        ...
        loop {
            ...
            if *closed { let _ = self.mtx.unlock(); return Err(Error::QueueClosed); }
            if let Some(item) = buffer.pop_front() { out.copy_from_slice(&item); self.not_full.signal(); let _ = self.mtx.unlock(); return Ok(()); }
            if !self.not_empty.timedwait(&self.mtx, &deadline) { let _ = self.mtx.unlock(); return Err(Error::Timeout); }
        }
    }
    ...
    fn fetch_from_isr(&self, out: &mut [u8]) -> Result<()> {
        ...
        if out.len() != self.message_size { let _ = self.mtx.unlock(); return Err(Error::InvalidMessageSize); }
        let buffer = unsafe { &mut *self.buffer.get() };
        if let Some(item) = buffer.pop_front() { out.copy_from_slice(&item); self.not_full.signal(); let _ = self.mtx.unlock(); Ok(()) }
        else { let _ = self.mtx.unlock(); Err(Error::Timeout) }
    }

    fn delete(&mut self) { self.close(); }
}

impl Drop for Queue { fn drop(&mut self) { self.close(); } }

impl Deref for Queue { type Target = QueueHandle; fn deref(&self) -> &Self::Target { &self.handle } }
...
impl Debug for Queue {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        ...
        let _ = self.mtx.unlock();
        f.debug_struct("Queue").field("len", &len).field("capacity", &self.capacity).finish()
    }
}
...
impl<T: StructSerde> QueueStreamed<T> {
    pub fn new(size: UBaseType, msg_size: UBaseType) -> Result<Self> { Ok(Self(Queue::new(size, msg_size)?, PhantomData)) }
...
impl<T: StructSerde> QueueStreamedFn<T> for QueueStreamed<T> {
    ...
    fn fetch(&self, buffer: &mut T, time: TickType) -> Result<()> {
        let mut buf = vec![0u8; buffer.len()]; self.0.fetch(&mut buf, time)?;
        *buffer = T::from_bytes(&buf)?; Ok(())
    }
    fn fetch_from_isr(&self, buffer: &mut T) -> Result<()> {
        let mut buf = vec![0u8; buffer.len()]; self.0.fetch_from_isr(&mut buf)?;
        *buffer = T::from_bytes(&buf)?; Ok(())
    }
...
impl<T: StructSerde> QueueStreamedFn<T> for QueueStreamed<T> {
    fn post(&self, item: &T, time: TickType) -> Result<()> {
        ...
        if w != buf.len() { return Err(Error::InvalidMessageSize); }
    ...
    fn fetch(&self, buffer: &mut T, time: TickType) -> Result<()> {
        let mut buf = vec![0u8; buffer.len()]; self.0.fetch(&mut buf, time)?;
        *buffer = serde_from_bytes(&buf).map_err(|_| Error::Unhandled("de"))?; Ok(())
    }
    fn fetch_from_isr(&self, buffer: &mut T) -> Result<()> {
        let mut buf = vec![0u8; buffer.len()]; self.0.fetch_from_isr(&mut buf)?;
        *buffer = serde_from_bytes(&buf).map_err(|_| Error::Unhandled("de"))?; Ok(())
...
impl<T: StructSerde> Deref for QueueStreamed<T> { type Target = Queue; fn deref(&self) -> &Self::Target { &self.0 } }
impl<T: StructSerde> Debug for QueueStreamed<T> {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result { f.debug_struct("QueueStreamed").field("q", &self.0).finish() }
}
impl<T: StructSerde> Display for QueueStreamed<T> {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result { write!(f, "QueueStreamed {{ {} }}", self.0) }
}
